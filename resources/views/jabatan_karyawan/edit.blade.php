                            ...
                            <div class="col-sm-4">
                                <select name="jabatan_id" class="form-control" required>
                                    ...
                                    @foreach ($jabatan as $j)
                                        <option value="{{ $j->id }}" 
                                            {{ (isset($jabatanKaryawan) && $jabatanKaryawan->jabatan_id == $j->id) || old('jabatan_id') == $j->id ? 'selected' : '' }}>
                                            {{ $j->nama_jabatan }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-sm-4">
                                <select name="karyawan_id" class="form-control" required>
                                    ...
                                    @foreach ($karyawan as $k)
                                        <option value="{{ $k->id }}" 
                                            {{ (isset($jabatanKaryawan) && $jabatanKaryawan->karyawan_id == $k->id) || old('karyawan_id') == $k->id ? 'selected' : '' }}>
